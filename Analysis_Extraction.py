...
from Analysis_RegionSplit import *

class ContourExtraction:
    def __init__(self, full_image, blue_image):
        """
        Initialize the ContourExtraction.
    ...
    def reorder_contour_points(self, contour, mask):
        ...
        if len(segments) > 1:
            ...
            reordered_contour = segments[0].copy() if segments else None

        return reordered_contour, segments

    def visualize_contours(self, full_contours, valid_parts_contours, segment_contours, mask=None, valid_parts=None):
        """
        ...
        Args:
            ...
            mask (numpy.ndarray): Buffered mask.
    ...
    def extract_contours_by_arbitrary_regions(self, mask, n_vertical=1, n_horizontal=1, thresholds=None, morph_kernel=(5, 5), smooth=False):
        """
        ...
        # Convert full image to grayscale if needed
        if len(self.full_image.shape) == 3:
            gray_image = cv2.cvtColor(self.full_image, cv2.COLOR_BGR2GRAY)
        else:
            gray_image = self.full_image.copy()
        ...
        for i in range(n_horizontal):
            for j in range(n_vertical):
                ...
                region_image = gray_image[y_start:y_end, x_start:x_end]
                ...
                for k, contour in enumerate(contours):
                    ...
                    if reordered is not None:
                        for segment in segments:  # Adjust coordinates to the full image scale
                            segment[:, 0, 0] += x_start
                            segment[:, 0, 1] += y_start
                        valid_parts_contours.extend(segments)
                        segment_contours.append(segments)

                    # Store full contour if it intersects the buffer
                    if any(cleaned_mask[py, px] > 0 for px, py in contour[:, 0]):
    ...
    def extract_contours_by_bounding_box(self, mask, min_size, thresholds=None, morph_kernel=(5, 5), smooth=False):
        """
        Extract contours separately by bounding box, applying different thresholds per region.

        Args:
            mask (numpy.ndarray): Binary mask defining the region of interest.
            min_size (int): Stop splitting bounding box if region is smaller than this size
            thresholds (list of list of tuple or None): 2D list where each entry corresponds to a region's (min_threshold, max_threshold).
                If None, Otsu’s method is applied per region.
            morph_kernel (tuple): Kernel size for morphological operations.
            smooth (bool): Whether to smooth the contours.
        ...
        decomposer = AdaptiveRegionDecomposition(self.full_image, mask, min_size=min_size)
        box_regions = decomposer.output_regions()

        # Check if thresholds are provided, otherwise use Otsu's method
        if thresholds is None:
            thresholds = [None for _ in range(len(box_regions))]  # Placeholder for Otsu

        if len(thresholds) != len(box_regions):
            raise ValueError("Thresholds must match the number of bounding box.")
        ...
        for i in range(len(box_regions)):
            # Define region coordinates
            x_start, x_end = box_regions[i][0],box_regions[i][2]
            y_start, y_end = box_regions[i][1],box_regions[i][3]
        ...
        print(f"Extracted {len(valid_parts_contours)} valid parts, {len(full_contours)} full contours, and {len(segment_contours)} segmented contours from {len(box_regions)} bounding boxes.")
        return valid_parts_contours, full_contours, segment_contours

    def connect_nearest_contours(self, valid_parts_contours):
        """
        Connect contours by finding the nearest start and end points and connecting them with a straight line.

        Args:
            valid_parts_contours (list of numpy.ndarray): List of contours.

        Returns:
            list of numpy.ndarray: Connected valid_parts_contours.
        """
        current_contour = valid_parts_contours.pop(0)
        current_contour_points = current_contour.reshape(-1, 2)
        start_point = tuple(current_contour_points[0])
        end_point = tuple(current_contour_points[-1])

        to_merge = []
        for i, contour in enumerate(valid_parts_contours):
            contour_points = contour.reshape(-1, 2)
            contour_start_point = tuple(contour_points[0])
            contour_end_point = tuple(contour_points[-1])

            # Calculate distances between current contour's end and next contour's start/end
            dist_end_to_start = np.linalg.norm(np.array(end_point) - np.array(contour_start_point))
            dist_end_to_end = np.linalg.norm(np.array(end_point) - np.array(contour_end_point))

            # Calculate distances between current contour's start and next contour's start/end
            dist_start_to_start = np.linalg.norm(np.array(start_point) - np.array(contour_start_point))
            dist_start_to_end = np.linalg.norm(np.array(start_point) - np.array(contour_end_point))

            to_merge.append((i, dist_end_to_start, dist_end_to_end, dist_start_to_start, dist_start_to_end))

        # Sort the contours to merge based on the smallest distance
        to_merge.sort(key=lambda x: min(x[1], x[2], x[3], x[4]))

        i, dist_end_to_start, dist_end_to_end, dist_start_to_start, dist_start_to_end = to_merge[0]
        contour = valid_parts_contours[i]
        contour_points = contour.reshape(-1, 2)

        # Determine the order to merge
        if dist_end_to_start == min(dist_end_to_start, dist_end_to_end, dist_start_to_start, dist_start_to_end):
            current_contour_points = np.vstack((current_contour_points, contour_points[1:]))
        elif dist_end_to_end == min(dist_end_to_start, dist_end_to_end, dist_start_to_start, dist_start_to_end):
            current_contour_points = np.vstack((current_contour_points, contour_points[-2::-1]))
        elif dist_start_to_start == min(dist_end_to_start, dist_end_to_end, dist_start_to_start, dist_start_to_end):
            current_contour_points = np.vstack((contour_points[-2::-1], current_contour_points))
        elif dist_start_to_end == min(dist_end_to_start, dist_end_to_end, dist_start_to_start, dist_start_to_end):
            current_contour_points = np.vstack((contour_points[1:], current_contour_points))

        # Remove the merged contour from valid_parts_contours
        valid_parts_contours.pop(i)

        # Add the merged contour to the result
        valid_parts_contours.append(current_contour_points)

        return valid_parts_contours
