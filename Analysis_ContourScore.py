...
def get_score(video_path, input_folder, output_folder, model, device):
    ...
    score_img_path = f'{output_folder}/0721test'
    contour_img_path = f'{output_folder}/contour'
    os.makedirs(score_img_path, exist_ok=True)
    os.makedirs(contour_img_path, exist_ok=True)
    ...
    # pred_image_files = ['frame_0088.jpg']
    for filename in pred_image_files:
        ...
        # contours, thresh = utils.process_region(roi_gray)
        # cv2.imshow('thresh',thresh)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()

        # utils.visualize_contours_on_image(roi_gray, contours)

        save_path = f'{contour_img_path}/{base_name}_contours.jpg'
        contour = utils.extract_vessel_wall_boundary(roi_gray, save_path)
